...
select course.cname, teacher.tname from course join teacher on teacher.tid=course.teacher_id;
-- 4、查询平均成绩大于八十分的同学的姓名和平均成绩
select student.sname,avg(number) from score join student on student.sid = score.student_id group by student.sname having avg(number) > 80;
-- 7、 查询没有报李平老师课的学生姓名
select distinct sname from student join score on score.sid = student.sid where score.cid not in
(select cid from teacher join course on teacher.tid=course.teacher_id where teacher.tname='李平';)
-- 8、 查询没有同时选修物理课程和体育课程的学生姓名
--     (只要选了一门的 选了两门和没有选的都不要)
select student.sname from student join score on student.sid = score.student_id where score.course_id in(
    select cid from course where cname in ("物理","体育")
) group by student.sid having count(*) = 1;
-- 9、 查询挂科超过两门(包括两门)的学生姓名和班级
select student.sname, class.caption from student join class on class.cid = student.class_id where student.sid in(
    select student_id from score where number < 60 group by student_id having count(student_id) > 2
);
```
